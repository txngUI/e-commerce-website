...
class ApiFeatureContext extends TestCase implements Context 
{
    ...
    /**
     * @Given /^(?:|I )change uri base to "(?P<uri>[^"]+)"$/
     */
    public function iChangeUriBase($uri)
    {
       $this->client = new Client([
            'base_uri' => $uri,
            'http_errors' => false
    ...
    /**
     * @Given /^I authenticate with ck "([^"]*)" and cs "([^"]*)"$/
    ...
    public function theHeaderShouldBe($headerName, $expectedHeaderValue)
    {
        ...
        // https://docs.guzzlephp.org/en/stable/psr7.html?highlight=headers#headers
        $this->assertEquals($expectedHeaderValue, (string) $response->getHeader($headerName)[0]);
    }
    ...
    public function thePropertyExists($property)
    {
        ...
        $this->assertTrue($this->arrayHas($payload, $property), $message);
    }
    ...
    public function thePropertyDoesNotExist($property)
    {
        ...
        $this->assertFalse($this->arrayHas($payload, $property), $message);
    }
    ...
    /**
     * @Given /^print last response of api$/
     */
